...
--- A minimal checkbox, which can be checked or unchecked.
--
-- @type Checkbox
local Checkbox = class "artist.gui.core.Checkbox"

function Checkbox:initialise(options)
  expect(1, options, "table")

  self.x = field(options, "x", "number")
  self.y = field(options, "y", "number")
  self.label = field(options, "label", "string")
  self.on_change = field(options, "on_change", "function", "nil")
  self.checked = field(options, "checked", "boolean", "nil") or false
  self.fg = field(options, "fg", "string", "nil") or "white"
  self.bg = field(options, "bg", "string", "nil") or "cyan"
  self._focused = false
end

Checkbox.attach, Checkbox.focus, Checkbox.blur = basic_attach, basic_focus, basic_blur

function Checkbox:draw(term, palette)
  local checkmark = self.checked and "x" or " "
  
  -- Draw the label followed by the checkbox
  term.setCursorPos(self.x, self.y)
  write_with(term, self.label .. " [" .. checkmark .. "]", palette[self.fg], palette[self.bg])
end

function Checkbox:handle_event(event)
  if event[1] == "mouse_click" then
    local button, x, y = event[2], event[3], event[4]
    local checkbox_width = #self.label + 4  -- Total clickable width: label + " [ ]"
    if button == 1 and x >= self.x and x < self.x + checkbox_width and y == self.y then
      self.checked = not self.checked  -- Toggle checked state
      self:mark_dirty()

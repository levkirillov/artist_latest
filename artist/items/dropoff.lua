...
return function(context)
  ...
  local config = context.config
    ...
    :define("used_by_user", "The chests where user places items(need for crafting handling).", {}, schema.list(schema.peripheral))
    :get()
      ...
      for i = 1, #config.chests do
        ...
        if contents then
          for slot, item in pairs(contents) do
            picked_any = true
            if (function()
                for _, used_us in ipairs(config.used_by_user) do
                  if chest == used_us then 
                    return false
                  end
                end
                return true
              end)() then context.mediator:publish("dropoff.add", item) end
